		...
		if ($this->ion_auth_model->forgotten_password($identity))
		{
			...
			if ($user)
			{
				$data = array(
					...
					if($this->config->item('use_mandrill', 'ion_auth'))
					{
						if(!class_exists('Mandrill')) {
							die('Use composer to install Mandrill API php wrapper');
						}
						
						try {
							$mandrill = new Mandrill($this->config->item('mandrill_api_key', 'ion_auth'));
							$template_name = $this->config->item('mandrill_forgot_template', 'ion_auth');
							$template_content = array();
							$message = array( 
							'subject' => 'Activate your account',
							'from_email' => 'office@hellgaming.ro',
							'from_name' => 'The Great British Raffle',
							'to' => array(
							array(
							'email' => $user->email,
							'name' => 'Recipient Name',
							...
							'headers' => array('Reply-To' => 'message.reply@example.com'),
								'global_merge_vars'	=>	array(
									array('name' => 'fname', 'content' => 'Mr. Example Name'),
									array('name' =>	'url', 'content'	=>	base_url('reset-password/' . $data['forgotten_password_code']))
								)
							);
							$async = false;
							$ip_pool = 'Main Pool';
							...
							$statuses = array('rejected', 'invalid');
							if(!in_array($result[0]['status'], $statuses, true)) {
								
								$this->set_message('forgot_password_successful');
								// redirect($this->config->item('mandrill_forgot_url', 'ion_auth'));
							} else {
							$this->set_error('forgot_password_unsuccessful');
								// redirect($this->config->item('mandrill_forgot_fail_url', 'ion_auth'));
							}
							/*
							Array
							(
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array())
	{
		...
		else
		{
			...
			if($this->config->item('use_mandrill', 'ion_auth'))
			{
				...
				try {
					$mandrill = new Mandrill($this->config->item('mandrill_api_key', 'ion_auth'));
					$template_name = $this->config->item('mandrill_activation_template', 'ion_auth');
					$template_content = array(
						array(
							'name' => 'FNAME',
							'content' => 'Mr. Example Name'
						),
						array(
							'name'		=>	'ACTIVATION',
						...
						array(
							'name'	=>	'URL',
							'content'	=>	base_url('activate/' . $activation_code)
						)
					);
					$message = array( 
					'subject' => 'Activate your account',
					...
					'headers' => array('Reply-To' => 'message.reply@example.com'),
						'global_merge_vars'	=>	array(
							array('name' => 'fname', 'content' => 'Mr. Example Name'),
							array('name' =>	'ACTIVATION', 'content'	=>	$activation_code),
							array('name' =>	'URL', 'content' =>	base_url('activate/' . $activation_code))
						)
					);
					$async = false;
					$ip_pool = 'Main Pool';
					...
					$statuses = array('rejected', 'invalid');
					if(!in_array($result[0]['status'], $statuses, true)) {
						redirect($this->config->item('mandrill_activation_url', 'ion_auth'));
					} else {
						redirect($this->config->item('mandrill_activation_fail_url', 'ion_auth'));
					}
					/*
					Array
					(
